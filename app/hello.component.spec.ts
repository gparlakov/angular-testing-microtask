...
describe('Integration::HelloComponent', () => {
  ...
  it('should call auth service getUser method on login', () => {
    ...
    const { comp, auth } = setup()
      .withUser(Promise.resolve(User.Empty));
  ...
  it('should redirect to login if getUser returns the empty user', () => {
    // pending('Remove this line to see this break, even though we follow the components logic')
    // given
    const { comp, router } = setup()
      .withUser(Promise.resolve(User.Empty));


    // instruct the navigate to log out to console to prove it actually does get called
    router.navigate.and.callFake(() => console.log('navigating'));
    // when
    comp.login();
